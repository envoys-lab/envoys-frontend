...
import { Text, Flex, Box, Skeleton, useMatchBreakpoints, ArrowBackIcon, ArrowForwardIcon } from '@envoysvision/uikit'
...
const DataRow: React.FC<{ tokenData: TokenData; index: number }> = ({ tokenData, index }) => {
  ...
  return (
    <LinkWrapper to={`/info/token/${tokenData.address}`} style={{ width: '100%' }}>
      <ResponsiveGrid>
        <Flex>
          <Text>{index + 1}</Text>
        </Flex>
