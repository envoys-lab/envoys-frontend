...
import MainEnvoysBunnyCard from './MainEnvoysBunnyCard'
import ManageEnvoysBunniesCard from './ManageEnvoysBunniesCard'
...
interface IndividualEnvoysBunnyPageProps {
  bunnyId: string
}

const IndividualEnvoysBunnyPage: React.FC<IndividualEnvoysBunnyPageProps> = ({ bunnyId }) => {
  ...
  const { isUpdatingEnvoysBunnies, latestEnvoysBunniesUpdateAt, fetchMoreEnvoysBunnies } =
  ...
  useEffect(() => {
    ...
    const msSinceLastUpdate = Date.now() - latestEnvoysBunniesUpdateAt
    // Check for last update is here to prevent too many request due to fetchMoreEnvoysBunnies updating too often
    ...
    if (msSinceLastUpdate > PANCAKE_BUNNIES_UPDATE_FREQUENCY && !isUpdatingEnvoysBunnies && isWindowVisible) {
      fetchMoreEnvoysBunnies(priceSort)
    }
  }, [
    priceSort,
    fetchMoreEnvoysBunnies,
    isUpdatingEnvoysBunnies,
    latestEnvoysBunniesUpdateAt,
    fastRefresh,
    isWindowVisible,
  ])
  ...
  }, [fetchMoreEnvoysBunnies, priceSort, previousPriceSort])
    ...
    <Page>
      ...
      <TwoColumnsContainer flexDirection={['column', 'column', 'row']}>
        <Flex flexDirection="column" width="100%">
          <ManageEnvoysBunniesCard bunnyId={bunnyId} lowestPrice={cheapestBunny?.marketData?.currentAskPrice} />
        ...
        <ForSaleTableCard
          ...
          loadMore={fetchMoreEnvoysBunnies}
          priceSort={priceSort}
          togglePriceSort={togglePriceSort}
          isFetchingMoreNfts={isUpdatingEnvoysBunnies}
        />
      </TwoColumnsContainer>
...
export default IndividualEnvoysBunnyPage
