...
const CompanyButton = ({holders, homePageUrl, className} : CompanyButton) => {
  ...
  return (
    <div className={`${styles['сompany-button']} ${className}`}>
      <div
        className={styles['сompany-button__button']}
        onClick={onTrade}
        >TRADE</div>
      <div className={styles['сompany-button__holders']}>
        ...
        <div
          className={styles['сompany-button__home-page-button']}
          onClick={onCompanyUrlClick}
        >{getClearDomian(homePageUrl)}<Link className={styles['link-icon']} color="#133D65" /></div>
      </div>
    </div>
  )
}
