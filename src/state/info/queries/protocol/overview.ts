...
const formatPancakeFactoryResponse = (rawPancakeFactory?: EnvoysFactory) => {
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      const overviewData = formatPancakeFactoryResponse(data?.envoysFactories?.[0])
      const overviewData24 = formatPancakeFactoryResponse(data24?.envoysFactories?.[0])
      const overviewData48 = formatPancakeFactoryResponse(data48?.envoysFactories?.[0])
